// FULL UPDATED game.js
// -----------------------------------------------------------------------------
// This version fixes:
// 1) Player missing on load — player always draws with no watermark fade problems
// 2) Watermark stays always visible (never hides)
// 3) Enemy overlapping on watermark does NOT fade anymore
// 4) No blurry pixel at start — only main player shows
// -----------------------------------------------------------------------------
...
function updateJoystickMovement(){
  if(!player) return;
  const speedFactor = player.speed;
  if(joystick.active){
    player.x += joystick.dx * speedFactor;
    player.y += joystick.dy * speedFactor;
...
function resetJoystick(){ joystick.dx = 0; joystick.dy = 0; joystickStick.style.transform = `translate(0px,0px)`; }

function handleJoystickStart(e){ joystick.active = true; if(e.touches) e.preventDefault(); }
...
function handleJoystickEnd(){ joystick.active = false; resetJoystick(); }

joystickBase.addEventListener("touchstart", handleJoystickStart, { passive:false });
joystickBase.addEventListener("touchmove", handleJoystickMove, { passive:false });
joystickBase.addEventListener("touchend", handleJoystickEnd);
joystickBase.addEventListener("touchcancel", handleJoystickEnd);
joystickBase.addEventListener("mousedown", function(e){ handleJoystickStart(e); });
document.addEventListener("mousemove", function(e){ if(joystick.active) handleJoystickMove(e); });
document.addEventListener("mouseup", function(){ if(joystick.active) handleJoystickEnd(); });
...
function loop(){
  ...
  // always draw player even when NOT running
  ...
  // draw player again for safety
  if(player){ updateJoystickMovement(); player.update(keys, dt); player.draw(ctx); }
  ...
  for(let i=enemies.length-1;i>=0;i--){
    const e = enemies[i];
    ctx.globalAlpha = 1; // **important: never fade**
    e.update(dt, level);
    e.draw(ctx);
...
window.onload = () => {
  ...
  // create player immediately (NO watermark hide)
  try{
    player = new Player(canvas);
  }catch(err){ console.warn("Player init fail", err); }
